...
function setDelay(difficulty) {
  ...
  if (difficulty === "easy") {
    ...
    // Handle unsupported difficulty levels or return a default value
...
function chooseHole(holes) {
  ...
  if (hole === lastHole) {
    // Call chooseHole recursively to pick a new random hole
    return chooseHole(holes);
  }

  // Update lastHole and return the selected hole
  lastHole = hole;
  return hole;
}
...
function gameOver() {
  // TODO: Write your code here
  if (time > 0) {
    var timeoutId = showUp(); // Call showUp() to continue the game
    return timeoutId;
  } else {
    var gameStopped = stopGame(); // Call stopGame() to end the game
    return gameStopped;
  }
}
...
function clearScore() {
  ...
  // Return the updated points value (which is now 0)
  return points;
}
...
function updateTimer() {
  ...
  if (time > 0) {
    time -= 1;
    // Update the timer display in the HTML using timerDisplay.textContent
...
function whack(event) {
  // TODO: Write your code here.
  // call updateScore()
  // Call the updateScore function to increment the points when a mole is whacked
  updateScore();
  return points;
}
